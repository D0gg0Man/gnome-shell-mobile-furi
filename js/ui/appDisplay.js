...
if (!this.mapped)
        ...
        if (placeholder instanceof AppViewItem) {
            ...
            if (!this._dragMaybeSwitchPageImmediately(clonedEvent)) {
                const {targetActor} = clonedEvent;

                if (targetActor === this._prevPageIndicator ||
                    targetActor === this._nextPageIndicator)
                    this._maybeSetupDragPageSwitchInitialTimeout(clonedEvent);
                else
                    this._resetDragPageSwitch();
            }
        }
...
*/
        ...
        longPressGesture.connect('end', () => {
            this._menu.modal = true;
        });
